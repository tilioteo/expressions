...
public class ExpressionFactory implements Serializable {
	...
	private static final Set<Character> PROHIBITED = new ReadOnlyHashSet<Character>(StringConstants.CHR_REV_QUOTE,
			StringConstants.CHR_AT, StringConstants.CHR_PERCENT, StringConstants.CHR_SQUARE_BRACE_OPEN,
			StringConstants.CHR_SQUARE_BRACE_CLOSE, StringConstants.CHR_CURLY_BRACE_OPEN,
			StringConstants.CHR_CURLY_BRACE_CLOSE, StringConstants.CHR_COLON, StringConstants.CHR_SEMI_COLON,
			StringConstants.CHR_QUOTE, StringConstants.CHR_BCK_SLASH, StringConstants.CHR_QUESTION);

	private static final Set<Character> SEPARATORS = new ReadOnlyHashSet<Character>(StringConstants.CHR_TAB,
			StringConstants.CHR_SPACE);

	private static final Set<Character> NUMBERS = new ReadOnlyHashSet<Character>(StringConstants.CHR_0,
			StringConstants.CHR_1, StringConstants.CHR_2, StringConstants.CHR_3, StringConstants.CHR_4,
			StringConstants.CHR_5, StringConstants.CHR_6, StringConstants.CHR_7, StringConstants.CHR_8,
			StringConstants.CHR_9);

	private enum Parenthesis {
		OPEN(StringConstants.CHR_BRACE_OPEN), CLOSE(StringConstants.CHR_BRACE_CLOSE);
	...
	private static OperatorNodeGroup getGroups(final String text) throws ExpressionException {
		...
		for (int i = 0; i < text.length(); ++i) {
			if ((i > 0) && (i < text.length() - 2) && (text.substring(i, i + Operator.DECLASS.getString().length())
					.equals(Operator.DECLASS.getString()))) {
	...
	private static boolean isValue(String string) {
		return (!Strings.isNullOrEmpty(string) && (NUMBERS.contains(string.charAt(0))
				|| (string.startsWith(StringConstants.STR_DOUBLE_QUOTE)
						&& string.endsWith(StringConstants.STR_DOUBLE_QUOTE))
